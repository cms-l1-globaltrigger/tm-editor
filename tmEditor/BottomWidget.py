...
from tmEditor.CommonWidgets import IconLabel, EtaCutChart, PhiCutChart
from tmEditor.ObjectEditor import CountObjects
...
class BottomWidget(QtGui.QWidget):
    ...
    def loadAlgorithm(self, algorithm, menu):
        ...
        content.append(self.tr("<p><code>%1</code></p>").arg(Toolbox.fAlgorithm(algorithm.expression)))
        ...
        if algorithm.objects():
            ...
            for object in algorithm.objects():
                ...
                if object_.type in CountObjects:
                ...
                content.append(self.tr("<li><img src=\":/icons/%1.svg\"> %2 (%3 %4, %5 BX offset)</li>").arg(object_.type.lower()).arg(object_.name).arg(comparison).arg(threshold).arg(bxOffset))
            content.append(self.tr("</ul>"))
            content.append(self.tr("</p>"))
        # List used objects.
        if algorithm.cuts():
            ...
            for cut in algorithm.cuts():
                ...
                if cut_.data:
                    content.append(self.tr("<li><img src=\":/icons/default.svg\"> %1 (%2)</li>").arg(cut).arg(cut_.data))
                else:
                    content.append(self.tr("<li><img src=\":/icons/default.svg\"> %1 (%2 to %3)</li>").arg(cut).arg(Toolbox.fCut(cut_.minimum)).arg(Toolbox.fCut(cut_.maximum)))
            content.append(self.tr("</ul>"))
            content.append(self.tr("</p>"))
