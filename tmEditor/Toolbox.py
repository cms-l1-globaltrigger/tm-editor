...
def loadXml(filename, location = getenv('TMGUI_DIR')):
    ...
    logging.info("loading XML file `%s'", filename)

    # Create a temporary directory and change current location.
    tmpdir = tempfile.mkdtemp()
    logging.info("created temporary location `%s'", tmpdir)
    cwd = os.getcwd()
    os.chdir(tmpdir)
    ...
    for src, dest in symlinks:
        os.symlink(src, dest)
        logging.info("created symbolic link `%s' -> `%s'", src, dest)
    ...
    logging.info("removed temporary location `%s'", tmpdir)
