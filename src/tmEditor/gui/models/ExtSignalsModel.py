...
class ExtSignalsModel(AbstractTableModel):
    ...
    def __init__(self, menu, parent: Optional[QtCore.QObject] = None) -> None:
        ...
        self.addColumnSpec("Cable", lambda item: item[kCable], format=int, textAlignment=self.AlignRight)
        self.addColumnSpec("Channel", lambda item: item[kChannel], format=int, textAlignment=self.AlignRight)
        self.addEmptyColumn()
