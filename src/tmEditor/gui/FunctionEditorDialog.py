...
def hasOverlapRemoval(name: str) -> bool:
...
class FunctionEditorDialog(QtWidgets.QDialog):
    ...
    def functionType(self) -> Optional[str]:
        """Returns function type."""
        comboBox = self.functionComboBox
        return comboBox.itemData(comboBox.currentIndex())

    def selectedCuts(self) -> list:
        """Retruns list of checked cut names."""
        selectedItems = list(filter(lambda item: item.checkState() == QtCore.Qt.Checked, self.cutModel._items)) # type: ignore
        return [item.data().name for item in selectedItems]

    def onUpdateObjectHelpers(self, index) -> None:
        """Update object helper widgets."""
        self.updateCuts()
        functionType = self.functionType()
        for helper in self.objectHelpers:
            # Enable all object types
            helper.types = list(ObjectTypes)
    ...
    def expression(self) -> str:
        ...
        if functionType:
            ...
            expression.addFunction(functionType, objects=objects, cuts=self.selectedCuts())
        return AlgorithmFormatter.normalize(expression.serialize())

    def loadFunction(self, token) -> None:
...
class FunctionReqHelper:
    ...
    def __init__(self, index, parent) -> None:
        ...
        self.types = list(ObjectTypes) # allowed object types
    ...
    def isValid(self) -> bool:
        ...
        return self.lineEdit.isEnabled() and len(self.text()) != 0
